...
router.post('/content/:contentId/generate-audio', async (req, res) => {
  ...
  try {
    ...
    // 如果执行到这里，说明 language !== 'en'，使用腾讯云TTS生成中文音频
    console.log('🔵 ========== 进入中文音频生成分支 ==========');
    console.log('🔵 确认 language 不是英文，language =', language);

    // 中文音频：使用腾讯云TTS
    console.log('🎵 调用腾讯云长语音合成API，文本长度:', text.length, '语言:', language);
    
    // 中文音色：1001-智逍遥（亲和女声），1002-智聆（亲和男声），1003-智言（亲和女声），1004-智娜（亲和女声）
    const voiceType = 1001; // 中文使用1001（智逍遥）
    console.log(`🎤 选择音色类型: ${voiceType} (中文-智逍遥)`);
    console.log(`📝 生成中文音频，文本长度: ${text.length}，内容预览: ${text.substring(0, 100)}...`);
    
    // 腾讯云TextToVoice API限制较严格（通常限制在200字符左右）
    // 对于中文音频，如果文本较长（>150字符），使用CreateTtsTask API（长文本语音合成）
    let responseData;
    
    // 对于中文，文本长度>150字符时使用长文本API
    const useLongTextAPI = text.length > 150; // 中文长文本使用CreateTtsTask API
    ...
    if (useLongTextAPI) {
      ...
      try {
        ...
        console.log('✅ 腾讯云API响应:', JSON.stringify(responseData, null, 2));
        ...
        if (responseData.Error) {
          ...
          if (error.Code === 'UnsupportedOperation.TextTooLong') {
            console.log('⚠️ TextToVoice API文本太长，切换到长文本API');
            throw new Error('TextTooLong'); // 抛出错误以触发切换到长文本API
          }
        ...
        // TextToVoice API返回的Audio是base64编码的音频数据，不是URL
        // 标记这是base64数据，后续需要解码
        responseData._isBase64 = true;
      } catch (error) {
        console.error('❌ TextToVoice API调用失败:', error);
        console.error('❌ 错误详情:', JSON.stringify(error, Object.getOwnPropertyNames(error)));
        ...
        // 检查是否是腾讯云API错误响应（SDK可能将错误包装在Error属性中）
        let shouldUseLongTextAPI = false;
        if (error.Error) {
          const apiError = error.Error;
          console.error('❌ 腾讯云API错误响应:', apiError);
          
          // 如果是文本太长错误，切换到长文本API
          if (apiError.Code && (apiError.Code.includes('TextTooLong') || apiError.Code.includes('TextToo'))) {
            shouldUseLongTextAPI = true;
            console.log('📝 TextToVoice API文本太长，切换到长文本API');
          } else {
            // 其他错误，直接返回
            return res.status(500).json({
              success: false,
        ...
        // 如果TextToVoice失败（包括TextTooLong错误），使用长文本API
        if (shouldUseLongTextAPI || error.message === 'TextTooLong' || (error.code && error.code.includes('TextTooLong'))) {
          ...
          console.log(`🔧 切换到长文本API，使用模型类型: ${modelType} (${modelType === 2 ? '精品模型' : '基础模型'})`);
          ...
          try {
            ...
            if (responseData.Error) {
              ...
              console.error('❌ 腾讯云长文本API错误:', apiError);
              ...
              return res.status(500).json({
                success: false,
                message: `腾讯云长文本API错误: ${apiError.Message || '未知错误'}`,
                error: apiError.Message || JSON.stringify(apiError),
                code: apiError.Code
              });
            }
          } catch (longTextError) {
            console.error('❌ 长文本API调用也失败:', longTextError);
            throw longTextError; // 重新抛出错误，让外层catch处理
          }
    ...
    if (responseData._isBase64) {
      // TextToVoice API返回的是base64编码的音频数据
      const base64Audio = responseData.Audio;
      if (!base64Audio) {
        throw new Error('腾讯云API响应中未找到音频数据');
      }
      
      console.log('✅ 检测到base64编码的音频数据，长度:', base64Audio.length);
      
      // 将base64字符串转换为Buffer
      buffer = Buffer.from(base64Audio, 'base64');
      console.log('✅ base64解码完成，Buffer长度:', buffer.length);
    } else {
